...
const formSchema = z.object({
  comments: z.string().min(1, "Комментарии обязательны"),
...
export function OtherServicesForm() {
  ...
  const onSubmit = async (values: z.infer<typeof formSchema>) => {
    ...
    try {
      const { data, error } = await supabase.from('tickets').insert({
        ...
        service_type: 'other',
        other_services_comments: values.comments,
        other_services_supplier: values.supplier,
        other_services_price: Number(values.price),
        other_services_paid: Number(values.paid),
        issue_date: new Date().toISOString(),
        travel_date: new Date().toISOString(),
        passenger_name: "Other Services",
        origin_city: "N/A",
        origin_code: "N/A",
        origin_country: "N/A",
        origin_name: "N/A",
      ...
      toast({
        title: "Успешно",
        description: "Дополнительные услуги успешно оформлены",
      });
      ...
      toast({
        variant: "destructive",
        title: "Ошибка",
        description: error.message || "Произошла ошибка при оформлении дополнительных услуг",
      });
    }
  };
  ...
  return (
    <div className="max-w-2xl mx-auto space-y-8">
      <div>
        <h1 className="text-3xl font-bold">Оформление дополнительных услуг</h1>
        <p className="text-muted-foreground mt-2">
          Заполните информацию о дополнительных услугах
        </p>
      </div>
